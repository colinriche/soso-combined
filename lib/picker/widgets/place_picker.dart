...
class PlacePickerState extends State<PlacePicker> {
    ...
    try {
      ...
      if (predictions.isEmpty) {
        ...
        for (dynamic t in predictions) {
          String description = t['description'].toLowerCase();

          // Check if the description contains the name of a UK city
          if (description.endsWith("uk") /* Add more cities as needed */) {
  ...
  LatLng final_location = LatLng(0, 0);
  ...
  Future<void> _delayedPop(BuildContext context) async {
    ...
    // Assume locationResult is already set when the user selects a location
    // If not, you may need to wait for the location to be set before calling _delayedPop
     Navigator.of(context).pop((locationResult?.latLng).toString());
     Navigator.of(context).pop((locationResult?.latLng).toString());
  }}
